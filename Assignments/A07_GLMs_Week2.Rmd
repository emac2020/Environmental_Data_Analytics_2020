...
PeterPaul.chem.physics.raw <- read.csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv")
...
```{r}
#3
NTL.chem.physics.wrangle <- PeterPaul.chem.physics.raw %>%
...
Temp.chem.physics.AIC <- lm(data = NTL.chem.physics.wrangle, temperature_C ~ year4 + daynum + depth)

step(Temp.chem.physics.AIC) 


Temp.chem.physics.Model <- lm(data = NTL.chem.physics.wrangle, temperature_C ~ year4 + daynum + depth)

summary(Temp.chem.physics.Model)
...
> Answer: The final set of explanatory variables that predict temperature is the set that contains year4, daynum, and depth, because it has the lowest AIC (26016.31). This model explains 74.17% of the variance.
...
Temp.ANCOVA.Interaction <- lm( data = NTL.chem.physics.wrangle, temperature_C ~ depth * lakename)
...
> Answer: Yes, there is a significant interaction between depth and lakename. This interaction explains 78.57% of the variance in the temperature observations. 
...
TempDepth.Graph <- 
  ggplot(NTL.chem.physics.wrangle, aes(x = depth, y = temperature_C, color = lakename)) +
  geom_point(alpha = .5) +
...
Litter.fixed <- gls(data = Litter,
                      dryMass ~ nlcdClass * functionalGroup)
summary(Litter.fixed)
rsquared(Litter.fixed)
...
> Answer: The mixed effects model is better than the fixed effects model because it has the lower AIC (9038.575).