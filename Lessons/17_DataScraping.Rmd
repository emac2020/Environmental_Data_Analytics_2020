...
# visit webpage, find nodes specific with path (web info we want), then read it as text
# td:nth-child(x): x = column number

State <- webpage %>% html_nodes("table:nth-child(20) td:nth-child(1)") %>% html_text()

Estuaries.Assessed.mi2 <- webpage %>% html_nodes("table:nth-child(20) td:nth-child(2)") %>% html_text() # areas of estuaries assessed

Estuaries.Assessed.percent <- webpage %>% html_nodes("table:nth-child(20) td:nth-child(3)") %>% html_text() # % assessed

Estuaries.Impaired.mi2 <- webpage %>% html_nodes("table:nth-child(20) td:nth-child(4)") %>% html_text() #area that are impaired

Estuaries.Impaired.percent <- webpage %>% html_nodes("table:nth-child(20) td:nth-child(5)") %>% html_text() # % impaired

Estuaries.Impaired.percent.TMDL <- webpage %>% html_nodes("table:nth-child(20) td:nth-child(6)") %>% html_text() # % impaired and TMDL

# Note: each of these objects has 23 items in it; consistency is good! Table needs same dimensions
...
> The more area assessed the more impaired
...
ggplot(Estuary, aes(x = Estuaries.Assessed.percent, y = Estuaries.Impaired.percent, 
  ...
  #geom_label_rebel(aes(label = State), nudge_x = -5, nudge_y = -5, 
                 # size = 3, alpha = 0.8)
  geom_label(aes(label = State), nudge_x = -5, nudge_y = -5, 
                  size = 3, alpha = 0.8) +  
...
ggplot(Estuary, aes(x = Estuaries.Assessed.mi2, y = Estuaries.Impaired.mi2, 
  ...
  geom_label(data = subset(Estuary, State %in% c("Virginia", "Florida", "Louisiana", "Texas")), 
              aes(label = State), nudge_x = -500, nudge_y = 200) +
