...
Please make sure that the pdf() command works on your computer. This helps us view the plots in pdf rather than "plots" window where it's super slow to look at large data files. Can see the file in this lesson folder --> sf lesson --> outputs on hard drive 
...
```{r activity.here}
# test out here. similar to getwd()
...
# use here to generate a data path to save or input something without using "./Data/...."
...
here("data", "tabular_data", "conterm_climate.txt")
...
1. points - a single coordinate combination of x and y (or longitude and latitude, respectively. can attach it to a point)
...
1. vector data - points, lines, polygons (things we can map with vector operations)
...
1. sf - the data frame that with feature attributes and feature geometries, this contains...sfc
2. sfc - the list-column with the geometries for each record, this contains...sfg
3. sfg - the feature geometry of an individual simple feature (coordinates for that simple feature)
...
There are two main ways to define a CRS in sf: (Can be done simultaneously)
...
1. [https://epsg.io/](http://www.epsg-registry.org/) - open source nicer user interface to access data in the EPSG registry, includes a little transformation calculator too! and proj4 string generator, this is my go-to CRS look-up database. Can grab the proj4 string under "Description" and import it into R
...
The data we'll use in this lesson comes from the United States Geologic Survey (USGS). Specifically, the dataset is called the Geospatial Attributes of Gages for Evaluating Steamflow, version 2 (GAGES-II) available [here](https://water.usgs.gov/GIS/metadata/usgswrd/XML/gagesII_Sept2011.xml). GAGES-II provides geospatial data and classifications of 9,322 stream gages that are maintained by the USGS. It includes: (1) watershed boundaries by region (spatial data), (2) USGS stream gage locations (spatial data), (3) watershed characteristics (tabular data). Note that the term "watershed" is used interchangeably with the words "basin" and "catchment".

#same gauge data we've been using for ENO, but expanding it to NC and the stuff below to add base layers
...
# Maps package is good for state boundaries, country, watersheds/basins, etc.
...
# note how I'm using the here package! Have 6 files for each main file (i.e., bas_nonref...) but only need to upload the .shp file
```
...
When *setting* a CRS, you're labeling the spatial data with a CRS it already has but is not showing up when you load the data into R. Setting also includes defining a missing ESPG or proj4 string; when one is complete but the other is not. Have CRS but not showing up when u load it in. You have a CRS, i know you have it, but you don't know you have it, so we have to define it.** use st_set_crs command. gives warning to show you you're just setting and not projecting (changing)

When *projecting* a CRS, you're converting the spatial data from one CRS to another. For the sf package, transform is used interchangeably with project. If have different EPSG codes or proj4 strings, can make them the same using project or transform. If CRS exists and you want to change it to interpret the spatial data a different way (e.g., focus on area instead of shape)
...
# add your code here. Both show up as values. first one has to be in quotes
...
st_crs(southeast_state_bounds_na_albers) # USGS version of data
```
...
head(gages_tabular_as_sf) # have a geometry column now
st_crs(gages_tabular_as_sf) # asks for CRS
...
# just grabs the geometry column 
...
** May want to buffer, summarize, merge/combine, calculate the area of polygons or the length of something (geometry)

Some common sf spatial operation commands might include (in no special order): (look at cheatsheet for more options)

1. st_transform() - We've seen this before. Projects from one CRS to another.
2. st_area() - Calculates the area of a polygon.
3. st_centroid() - Finds the centroid (x-y coordinate) of a polygon or combined geometry. i.e., if you want to show data at centroid of watershed instead of outlet, you can show the centroid point and color it by rainfall or something 

4. st_difference() - Removes overlapping spatial data between two datasets.

5. st_intersection() - Finds overlapping spatial data between two datasets where output is an sf object that's clipped to the boundary of the intersected dataset. **Think of as "clip" from ArcGIS can use this to clip the gauges that are in NC. 
...
8. st_buffer() - Creates a polygon extending from a geometry within a given distance. **May want to look at % landcover around 10m buffer around a stream
...
11. so many others...See cheat sheet for more details*****
...
Geometric operations: results will be spatial dataset to make into a shape file. this will be the actual gauge and attribute info

Geometric Confirmation: list of true and false values. R is confirming whether or not a gauge is in a watershed you're intersecting
...
nc_state_bounds_geom <- southeast_state_bounds %>%
  ...
  st_geometry() # this will drop the attribute data so it only has the geometry data without columns (dataframe). if look at class, it will now be different. no longer a data frame and instead a "sfc_MULTIPOLYGON" "sfc" these are subclasses of "sf"

# for virginia

va_state_bounds_geom <- southeast_state_bounds %>%
...
head(va_state_bounds_geom) # can see the data has same projection and proj4 string as before and is a multipolygon with XY dimension
...
va_basins_nf_seplains <- basins_nf_seplains %>%
  st_intersection(va_state_bounds_geom) #intersection is the same as "clip." cutting out all the basins inside virginia

# check
# add your code here
head(nc_basins_nf_seplains)

head(va_basins_nf_seplains) #Shows the watershed gauge ID associated with that watershed and the area and the perimeter


# select gages that fall within NC bounds
# add your code here
nc_gages <- gages %>%
  st_intersection(nc_state_bounds_geom) #gauges = cookie dough, state bounds = cookie cutter

va_gages <- gages %>%
  st_intersection(va_state_bounds_geom)

# bonus
# add your code here
nc_area <- st_area(nc_state_bounds_geom) # find the area of NC

va_area <- st_area(va_state_bounds_geom) # shows area in meters squared (105598646413m^2)

nc_centroid <- st_centroid(nc_state_bounds_geom) # find the centroid of the NC

va_centroid <- st_centroid(va_state_bounds_geom) # can use this as a spatial dataset to plot

nc_gages_buffer_100m <- st_buffer(nc_gages, dist = 100) # make a buffer around the stream gages in NC

va_gages_buffer_100m <- st_buffer(va_gages, dist = 100) # make a buffer with a 100m buffer

not_nc_gages <- st_difference(gages, nc_state_bounds_geom) # find stream gages not in NC

not_va_gages <- st_difference(gages, va_state_bounds_geom) # shows all the gauges that are not in virginia

# can run these lines of code but don't know if they're working until we can actually see the data. so should plot the result to make sure
```
...
```{r basic.sf.plotting}
# where we define data depends on where we want ggplot to set the base to. So leave ggplot() blank and add layers below
# then add the type of plot you want to make (i.e., scatter/point). here we're doing geom_sf and showing the boundaries as the bottom layer (geom_sf(data = bounds, aes(...))) or first layer and can set aes to this. then want points as the second layer after that (geom_sf(data = points))
...
ggplot(data = southeast_state_bounds) + # moved this up here so I don't have to keep calling it
  geom_sf() +
  geom_sf_text(aes(label = NAME), color = "black") + # want label to be name column in black
  theme_bw()
dev.off()
...
ggplot(data = southeast_state_bounds) +
  geom_sf(aes(fill = Shape_Area)) + #add color to the shapes (states in this case)
  geom_sf_text(aes(label = NAME), color = "white") + # want name to stand out so change it to white
  theme_bw()
dev.off()


# Just Virginia boundary to see if our VA boundary code from above actually worked 

# basic plot
pdf(here("outputs", "va_state_bounds.pdf"), width = 11, height = 8.5)
ggplot() +
  geom_sf(data = va_state_bounds_geom) + # you can add color = "blue" to change outline color to blue or lwd = 2 to change the width of the lines
  theme_bw() # add this to make the background white and not grey
dev.off()
```
...
ggplot() +
geom_sf(data = va_state_bounds_geom) +
  geom_sf(data = va_basins_nf_seplains, fill = "pink", alpha = 0.25) + # deeper pink = more watershed on top of each other
  geom_sf(data = va_gages, color = "black") 
dev.off()
...
nc_state_bounds_geom <- southeast_state_bounds %>%
  filter(NAME == "North Carolina") %>% # drop the geometry 
  st_geometry()

# get data bounding box: makes base map that shows more than just NC. buffer it by 150000m
nc_bbox <- nc_state_bounds_geom %>%
  ...
  st_transform(4326) %>% # WGS84 (for lat and long). translates it from albers back to lat and long
  st_bbox() # finds the bounding box edges' coordinates
nc_bbox # check

# have to change the nc_bbox coordinates into a setting that the map function likes

# fix bounding box columns so they match what is needed for the ggmap::get_map() function. has to have left, bottom, right, and top entry

nc_bbox_fix <- c(left = nc_bbox[[1]], bottom = nc_bbox[[2]], right = nc_bbox[[3]], top = nc_bbox[[4]])

# check result. changed the labels 
nc_bbox_fix

# get basemap
nc_basemap <- get_map(nc_bbox_fix, maptype = 'terrain-background', source = 'stamen', zoom = 8) # set map type (i.e. terrain-background, but can be different) and get the stamen basemap. other options like google base map, etc. and then zoom (can zoom from 3 [continent] to 20 [building] scale and defaults to 10 [city])

class(nc_basemap) # ggmap and raster
st_crs(nc_basemap)
# you'll have to trust me that this will have a CRS = WGS84

# convert (change/transform) nc_state_bounds_geom to WGS84 (lat/long) so it matches nc_basemap
...
ggmap(nc_basemap) + #using ggmap and setting to the basemap dataset
...
# gages data = meta data for gages and then have gages variable data (tabular dataset), so we have to spatial join the data to preserve spatial locations while adding another variable

#after join gages, can use that to plot and color by
...
names(my_tabular_data_raw) #basin info, climate, hydrology, geology, etc. can change the name of text file depending on what data u want to add
...
# join the tabular data to nc_gages. have to mutate it to make it the same type
