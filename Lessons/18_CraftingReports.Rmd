...
1. **YAML Header** surrounded by --- on top and bottom
    ...
    - want to include specific output (pdf, html, word, interactive options, more in cheatsheets below)
...
<Fill in our discussion below with bullet points. Use italics and bold for emphasis (hint: use the cheat sheets to figure out how to make bold and italic text).> ( anything in <...> don't show up in knitted doc)

* *italic* # can see different color for italic vs. bold
* **bold** 
* **Documentation** of data pipeline steps
* Store *code*, *visulations*, and *text* in one place
* **Organize** code into sections
* Aesthetically pleasing
* Manage changes in a *reproducible* fashion
...
Dataset Name    |  Dataset Notes
________________|________________
                |
Peter Paul Lake | Pollution
Cell 3          | Cell 4 
...
* ## creates headings that can be searched 
...
Lake.nutrients.raw <- read.csv("./Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv")
...
```{r, data exploration, echo = FALSE, eval = TRUE}
...
```{r, summary dataset, message = FALSE, echo = FALSE, eval = TRUE}

# Total N Summaries

NTL.TotalN.summaries <- 
  NTL.nutrient.data %>%
  group_by(lakename) %>%
  filter(!is.na(tn_ug)) %>%
  summarise(meanTN = mean(tn_ug),
            minTN = min(tn_ug),
            maxTN = max(tn_ug),
            sdTN = sd(tn_ug))
...
NTL.TotalP.summaries <- 
  NTL.nutrient.data %>%
  group_by(lakename) %>%
  filter(!is.na(tp_ug)) %>%
  summarise(meanTP = mean(tp_ug),
            minTP = min(tp_ug),
            maxTP = max(tp_ug),
            sdTP = sd(tp_ug))
```
...
knitr::kable(NTL.TotalN.summaries, caption = "Total N Summary Data")

# Total P Table
knitr::kable(NTL.TotalP.summaries, caption = "Total P Summary Data")
```
...
```{r Figure 1, fig.height = 4, fig.width = 10, fig.cap= 'Total N over time'}

# Total N Plot

TNplot <- 
  ggplot(NTL.nutrient.data, aes(x = sampledate, y = tn_ug, 
                     color = lakename)) +
  geom_point() +
  labs(x = expression(paste("Date")),
...
```{r Figure 2, fig.height = 4, fig.width = 10, fig.cap= 'Total P over time'}

# Total P Plot

TPplot <- 
  ggplot(NTL.nutrient.data, aes(x = sampledate, y = tp_ug, 
                     color = lakename)) +
  geom_point() +
  labs(x = expression(paste("Date")),
